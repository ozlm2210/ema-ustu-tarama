...
if 'scan_df' in st.session_state:
    ...
    # Excel indir: ekranda görünen temiz ondalıklarla .xlsx oluşturulur.
    excel_buffer = BytesIO()
    with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
        csv_df.to_excel(writer, sheet_name='EMA_USTU_%3', index=False)

        ws = writer.book['EMA_USTU_%3']
        ws.freeze_panes = 'A2'
        ...
        # Sayı biçimleri: fiyat 2, EMA 4, yüzde 2 basamak
        headers = {cell.value: cell.column for cell in ws[1]}
        for name, col_idx in headers.items():
            if name == 'Güncel Fiyat':
                for row_idx in range(2, ws.max_row + 1):
                    ws.cell(row_idx, col_idx).number_format = '0.00'
            elif name == 'En Yüksek EMA' or (isinstance(name, str) and '_EMA' in name and not name.endswith('_USTUNDE')):
                for row_idx in range(2, ws.max_row + 1):
    ...
    excel_buffer.seek(0)
    tarih = datetime.now(ZoneInfo('Europe/Istanbul')).strftime('%Y%m%d_%H%M')
    st.download_button(
        'SONUÇLARI EXCEL İNDİR',
        data=excel_buffer.getvalue(),
        file_name=f'EMA_USTU_TARAMA_{tarih}.xlsx',
