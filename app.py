...
def get_bist_symbols_and_prices():
    """
    TradingView Türkiye tarayıcısından:
      - BIST hisse listesini
      - her hissenin güncel/son 'close' fiyatını
    aynı istekte alır.

    ÖNEMLİ:
    "Güncel Fiyat" artık Yahoo'nun son günlük Close'u DEĞİL,
    TradingView scanner 'close' değeridir.
    """
    ...
    for row in js.get("data", []):
        ...
        if pd.notna(price) and price > 0:
            tv_prices[symbol] = price

    return sorted(symbols), tv_prices
...
def analyze_symbol(close, symbol, current_price):
    close = clean_close(close)

    if close.empty:
        return None

    # EMA geçmişi Yahoo ADJ kapalı verisinden hesaplanır.
    # Fakat filtrede kullanılan GÜNCEL FİYAT TradingView'den gelir.
...
def download_mode(symbols, tv_prices, progress_cb=None):
    results = []
    errors = []
    ...
    for start in range(0, total, CHUNK_SIZE):
        ...
        for ys in chunk:
            ...
            try:
                current_price = tv_prices.get(symbol, np.nan)
                    ...
                    errors.append(
                        {
                            "Aşama": "Güncel Fiyat",
                            "Hisse": symbol,
                            "Hata": "TradingView güncel close fiyatı alınamadı",
                        }
                    )
                    continue
                ...
                row = analyze_symbol(close, symbol, current_price)
...
st.info(
    "Güncel Fiyat: TradingView Türkiye tarayıcısı (close). "
    "EMA geçmiş verisi: Yahoo Finance (ADJ KAPALI). "
    "5Y Pivot/P: TradingView 12M."
)
...
if st.button(
    ...
    try:
        symbols, tv_prices = get_bist_symbols_and_prices()

        progress = st.progress(
            0.0,
            text=f"BIST listesi hazır: {len(symbols)} hisse | TradingView fiyatı: {len(tv_prices)}",
        )
...
if df is not None:
    if df.empty:
        ...
        st.caption(
            "Güncel Fiyat TradingView 'close' değeridir. "
            "P Mesafe % = ((TradingView Güncel Fiyat / 5Y P) - 1) × 100. "
